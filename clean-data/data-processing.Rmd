...
This data analysis serves as an example on how one might organize their annotation and high-dimensional omic data prior to leveraging the [spacemap R package](https://topherconley.github.io/spacemap/index.html) for network learning.  It is imperative to maintain the integrity of the Breast Cancer Proteogenomics Landscape Study (BCPLC) data through a transparent outline of every pre-processing step. Further, the biological annotation ought to accompany the data in a principled manner. This document details the pre-processing of the data. The result of this analysis is the input data used for network learning, analysis, and interpretation. We store the input data and annotation results together in [Bioconductor's](https://bioconductor.org/packages/release/bioc/html/Biobase.html) `BioBase::ExpressionSet` objects, which have been tailored to meet the needs of high-throughput data analysis. Finally, this analysis is entirely reproducible, but the initial data files are not included in the [neta-bcpls](https://topherconley.github.io/neta-bcpls/) repository to keep it as light-weight as possible in file sizes. If interested in reproducing this analysis with the initial data files, please contact the authors of spaceMap. 

## Proteogenomic Data Provenance 

In this document we process 105 matched breast tumour samples that are sourced from The Cancer Genome Atlas (TCGA).  Assays on these 105 samples generated protein quantification, mRNA expression, and copy number alteration (CNA) forming a framework for proteogenomic analysis. All proteogenomic data in evaluating `spacemap` was sourced from the [Breast Cancer Proteogenomics Landscape Study, Nature 2016](https://cptac-data-portal.georgetown.edu/cptac/s/S029) (BCPLS).The protein identification and quantitation data had been previously processed according to the Common Data Analysis Pipeline [(CDAP)](https://cptac-data-portal.georgetown.edu/cptac/aboutData/show?scope=dataLevels) of the of the Clinical Proteomics Tumor Analysis Consortium (CPTAC). Michael Gatza, an author of BCPLS, pre-processed the RNA-seq gene expression data set, which has been normalized, log transformed, median centered, and filtered to contain only those genes present in >80% of samples. Pei Wang, another author of BCPLS, called gains and losses in aCGH data with the ['Cluster along chromosomes algorithm'](http://biostatistics.oxfordjournals.org/content/6/1/45). 
...
kable(head(prot_gene_coordinates), row.names = F)
```
...
In the copy number alteration data, we found that even after processing with the 'Cluster along chromosomes algorithm' (CLAC), there were still groupings of highly-correlated copy number alteration intervals (CNA). We seek to further cluster these CNA so that the extreme multicolinearity does not render the multivariate regression unstable. 
...
To get a sense of how much spatial correlation affects the multicolinear clusters, we compute a distance on the CNA marker names. It is clear that most multicolinear clusters have the same interval name suggesting those intervals with similar distance will be clustered together. 
...
Those multicolinear CNA with interval names that are not identical, but are contiguous are renamed to reflect the contiguity. Only one of the multicolinear entries is not contiguous (index 59); but they happen to be in the same chromosome and the same arm. We make note of this by separating the two  cytoband regions with a semi-colon.  Lastly we rename the averaged multicolinear CNA with the representative interal name that spans two or more multicolinear CNA variables. 
...
Save the `ExpressionSet` objects, which contain the data to train the spaceMap model and perform downstream network analysis illustrated through `prot-net` and `RNA-net` examples in other sections of this website. 
...
Please see the [Gene Ontology](https://topherconley.github.io/neta-bcpls//neta/prep-go-universes.html) build for the next step in the analysis. 
