...
In this document we process 105 matched breast tumour samples that are sourced from The Cancer Genome Atlas (TCGA).  Assays on these 105 samples generated protein quantification, mRNA expression, and copy number alteration (CNA) forming a framework for proteogenomic analysis. All proteogenomic data in evaluating `spacemap` was sourced from the [Breast Cancer Proteogenomics Landscape Study, Nature 2016](https://cptac-data-portal.georgetown.edu/cptac/s/S029) (BCPLS).The protein identification and quantitation data had been previously processed according to the Common Data Analysis Pipeline [(CDAP)](https://cptac-data-portal.georgetown.edu/cptac/aboutData/show?scope=dataLevels) of the of the Clinical Proteomics Tumor Analysis Consortium (CPTAC). Michael Gatza, an author of BCPLS, was involved in the processed the RNA-seq gene expression data set, which has been normalized, log transformed, median centered, and filtered to contain only those genes present in >80% of samples. Pei Wang, another author of BCPLS, called gains and losses in aCGH data with the ['Cluster along chromosomes algorithm'](http://biostatistics.oxfordjournals.org/content/6/1/45). 
...
Record the high quality clinical information with samples sorted in the same order as RNA. 

```{r}
clin.hq <- clin[match(idMap$rnaOrder[idMap$highQual], clin$Sample),]
```

Subset the high quality samples.
...
The entrez gene identifiers are unique. 
...
Computational limits of `spaceMap` and `sCGGM` require us to filter out genes that do not exhibit high variance. Restrict the analysis to mRNA that exceed the 90th percentile in gene variation. 
...
The total number of mRNA expression considered for this analysis is: 

```{r}
nrow(rnatofit)
```

Mean-center mRNA features and scale by standard deviation. Follow this action by imputing mean-zero for missing values. 
...
head(rna_node_attributes)
#evidence of limited attributes
#rna_node_attributes[is.na(rna_node_attributes$strand),]
```
...
prot_node_attributes$description <- tmp$description
...
In the copy number alteration data, we found that even after processing with the 'Cluster along chromosomes algorithm' (CLAC), there were still groupings of highly-correlated copy number alteration intervals (CNAI). We seek to further cluster these CNAI so that the extreme multicolinearity does not render the multivariate regression unstable. 
...
+ **Was developed for spatial correlation clustering of database records and extends well to CNAI application.**
...
Load the `fpc` package for using the `dbscan` algorithm. Define the dissimilarity to be `1 - minus Correlation`. 
...
We require a minimally small cluster of size 2 and a `reacheablility` radius of `1e-3`. 
...
To get a sense of how much spatial correlation affects the multicolinear clusters, we compute a distance on the CNAI marker names. It is clear that most multicolinear clusters have the same interval name suggesting those intervals with similar distance will be clustered together. 

```{r}
suppressWarnings(library(stringdist))
library(foreach)
clusterDistr <- as.data.frame(table(clout$cluster), stringAsFactors = FALSE)
#Cluster `0` corresponds to sufficiently independent CNAI's. Omit it from the multi-colinear analysis.
...
hist(nameDist, col="#31a354", xlab = "restricted Damerau-Levenshtein distance of CNAI names", main = "" )
```
...
We will use the average of each mulitcolinear cluster to collapse (i.e. replace) the problematic CNAI into a single representative vector for future data analysis.
...
Almost all of these multicolinear CNAI are contiguous. We resolve the interval names of the multicolinear CNAI by either (1) collapsing the identical interval names or (2) changing the interval names to reflect an expanded contiguous range of the multicolinear CNAI. 
...
Those multicolinear CNAI with interval names that are not identical, but are contiguous are renamed to reflect the contiguity. Only one of the multicolinear entries is not contiguous (index 59); but they happen to be in the same chromosome and the same arm. We make note of this by separating the two  cytoband regions with a semi-colon.  Lastly we rename the averaged multicolinear CNAI with the representative interal name that spans two or more multicolinear CNAI variables. 
...
Cluster `0` corresponds to sufficiently independent CNAI's; these are combined with the collapsed multicolinear CNAI's.
...
The dimension of CNAI has now been reduced to 1662 from 1730 under this setting. 
...
Confirm that multicolinear CNAI have no direct overlaps in the genome. 
...
cna_node_attributes <- data.frame(id = cnaids,
                                 ...
                                 symbol = NA,
                                 cyto = nr_cytoband_ids,
                                 strand = NA, 
                                 type  = "x", 
                                 cytoscape = nr_cytoband_ids,
                                 stringsAsFactors = FALSE)
...
Save the `ExpressionSet` objects. 

```{r}
basedir2 <- "~/repos/neta-bccptac/data/"
